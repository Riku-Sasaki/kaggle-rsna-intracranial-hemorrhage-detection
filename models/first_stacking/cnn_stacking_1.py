    ...
    for i in range(5):
        ...
        for epoch in range(n_epoch):
            ...
            with torch.no_grad():
                ...
                metrics = []
                for i in range(6):
                    pred_valid_metric[:, i] = np.clip(pred_valid_metric[:, i], eps, 1 - eps)
                for i in range(6):
                    metrics.append(log_loss(np.floor(target_valid[:, i]), pred_valid_metric[:, i]))
                metrics = np.average(metrics, weights=[2, 1, 1, 1, 1, 1])
                print(f'Epoch[{epoch}]', 'Train:', np.mean(losses), 'Valid:', np.mean(losses_valid), 'Score', metrics)
            scheduler.step()
